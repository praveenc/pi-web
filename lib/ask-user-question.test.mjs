...
test("custom answers go through the Type something row and the follow-up input", async () => {
  ...
  assert.deepEqual(result, {
    cancelled: false,
    answers: [{ custom: "MySQL" }, { multi: "a hamster" }],
  });
});
...
test("drafts turn into answers the bridge accepts", async () => {
  ...
  // The typed answer is exclusive: stale checks never ride along with it.
  assert.deepEqual(draftToAnswer(multi, { ...emptyQuestionDraft(), checked: [1], custom: true, text: "y" }), { kind: "custom", text: "y" });
  assert.equal(draftToAnswer(multi, { ...emptyQuestionDraft(), custom: true, text: " " }), null);
  assert.deepEqual(draftToAnswer(multi, { ...emptyQuestionDraft(), checked: [1], text: "stale" }), { kind: "multi", optionIndexes: [1] });
...
test("a foreign dialog while the questionnaire is open does not end the bridge", async () => {
  const { QuestionnaireBridge } = await loadSubject();
  let release;
  ...
  const walk = walkLikeExtension(ui, QUESTIONS_ARGS);
  await ui.select("Another extension asks", ["yes", "no"]);
  release([{ kind: "option", optionIndex: 0 }, { kind: "multi", optionIndexes: [1] }]);

  assert.deepEqual(await walk, { cancelled: false, answers: [{ option: "Postgres" }, { multi: "2" }] });
  assert.deepEqual(nativeDialogs, ["Another extension asks"]);
});

test("typed multi-select text the extension would read as option numbers is refused", async () => {
  const { buildQuestionnaireScript, draftToAnswer, emptyQuestionDraft, isIndexOnlyText, parseAskUserQuestionArgs } = await loadSubject();
  const [single, multi] = parseAskUserQuestionArgs(QUESTIONS_ARGS);

  for (const text of ["2", "1 3", "1,3", "2."]) assert.equal(isIndexOnlyText(multi, text), true, text);
  for (const text of ["4", "0", "2 cats", "two", ""]) assert.equal(isIndexOnlyText(multi, text), false, text);

  assert.equal(draftToAnswer(multi, { ...emptyQuestionDraft(), custom: true, text: "2" }), null);
  assert.deepEqual(draftToAnswer(multi, { ...emptyQuestionDraft(), custom: true, text: "4" }), { kind: "custom", text: "4" });
  assert.equal(buildQuestionnaireScript([single, multi], [{ kind: "option", optionIndex: 0 }, { kind: "custom", text: "1,3" }]), null);
  // Single-select typed answers go through their own input and are never parsed.
  assert.deepEqual(draftToAnswer(single, { ...emptyQuestionDraft(), custom: true, text: "2" }), { kind: "custom", text: "2" });
});
